...
OPT_RESULT_EXPLAIN = {
    ...
    '1701': '对用户无任何权限',
    '1702': '无效的回调地址',
    '40002': '无权限操作该设备类型(产品型号)',
    '40009': '编解码过程出现错误',
    '100002': '注册设备达到上限',
    '100208': 'appId或secret错误',
    '100203': '应用不存在',
    '100418': '设备数据不存在',
    '100023': '数据中数据异常',
    '101001': '资源不存在',
...
REST_COMMON_DATA = {
    ...
    'command-cancel-tasks': {
        'uri': 'dev-control/',
        'rest_text': '创建撤销设备控制命令任务',
        'method': 'POST'
    },
    'query-command-cancel-tasks': {
        'uri': 'dev-control/',
        'rest_text': '设备控制命令撤销任务执行状态查询',
        'method': 'GET'
    }
}
...
class EasyIoT(object):
    ...
    @before_invoke_check
    def device_history_data(self):
        """查询设备业务数据

        """

        self.curr_rest = '查询设备业务数据接口'
        uri = 'device-history-data'
        request_body = {
            'devSerial': '',
            'startTime': '2018-04-08 00:00:00',
            'endTime': '2018-04-09 01:01:01',
            'pageNum': 1,
            'pageDataCount': 20
    ...
    @before_invoke_check
    def create_command_undo_task(self):
        """创建撤销设备控制命令任务

        """

        self.curr_rest = '创建撤销设备控制命令任务接口'
        uri = 'dev-control/command-cancel-tasks'
        request_body = {
            'devSerial': '设备序列号'
        }
        return self.common_requests('POST', uri, request_body)
    ...
    @before_invoke_check
    def query_command_undo_task(self, taskId):
        """设备控制命令撤销任务执行状态查询

        """

        self.curr_rest = '设备控制命令撤销任务执行状态查询接口'
        uri = 'dev-control/query-command-cancel-tasks'
        request_body = {
            'devSerial': '设备序列号',
            'taskId': taskId
        }
...
def main_test():
    ...
    # 16、创建撤销设备控制命令任务
    # rsp = iot.create_command_undo_task()
    #
    # taskId = rsp['taskId']

    # 17、查询指定设备,指定控制命令撤销任务的执行状态
    # iot.query_command_undo_task(taskId)
    ...
    # 查询设备业务数据
    # iot.common_method('device-history-data', startTime='2018-04-08 00:00:00', pageDataCount=20)

    # 创建撤销设备控制命令任务
    # iot.common_method('command-cancel-tasks', devSerial='')

    # 设备控制命令撤销任务执行状态查询
    # iot.common_method('query-command-cancel-tasks', devSerial='', taskId='')
